...
mkdir -p data/clean/qbt_subset1/all_reports

DATA_DIR=data/clean/qbt_subset1/busco/min1000bp
OUT_FILE=data/clean/qbt_subset1/all_reports/busco_min1000bp_filtered_report.txt
...
DATA_DIR=data/clean/qbt_subset1/tiara/min1000bp/
OUT_FILE=data/clean/qbt_subset1/all_reports/tiara_min1000bp_filtered_report.txt
...
OUT_FILE=data/clean/qbt_subset1/all_reports/quast_min1000bp_filtered_report.txt

HEADERS_SAMPLE=$(ls ${DATA_DIR} | grep 'min' | head -1)
HEADERS=$(cat ${DATA_DIR}/${HEADERS_SAMPLE}/transposed_report.tsv | head -1)

echo -e "Sample\t${HEADERS}" > ${OUT_FILE}

for SAMPLE in $(ls ${DATA_DIR} | grep '^min')
do
