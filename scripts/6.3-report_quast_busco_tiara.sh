...
DATA_DIR=data/clean/qbt/tiara/tiara_out
OUT_FILE=data/clean/qbt/all_reports/tiara_report.txt

for SAMPLE in $(ls ${DATA_DIR} | grep '^tiara_out')
do
...
HEADERS_SAMPLE=$(ls ${DATA_DIR} | grep 'quast_output' | head -1)
HEADERS=$(cat ${DATA_DIR}/${HEADERS_SAMPLE}/transposed_report.tsv | head -1)

echo -e "Sample\t${HEADERS}" > ${OUT_FILE}

for SAMPLE in $(ls ${DATA_DIR} | grep '^quast_output')
do
