...
getwd()
setwd("~/UVic/TFM/myfile")

rm(list = ls())

### Main directory

DATA_DIR <- "data/clean/quast_busco_tiara/all_reports/"     ## change directory
...
base$`Complete BUSCOs` <- busco$`Complete BUSCOs (C)`

base$`Fragmented BUSCOs` <- busco$`Fragmented BUSCOs (F)`
...
write_tsv(base, sprintf("%sQBT_summary_FINAL_SAG.tsv", DATA_DIR))